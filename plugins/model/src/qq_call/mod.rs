...
///
/// 实际可达这里的是主管理员与副管理员：命令在私聊分发层属于"受限命令"，非管理员发的
/// 会被静默丢弃（见 `model/private.rs`，这是有意的——外呼是给管理员的动作，通话授权
/// 名单只决定"谁能打进来"）。这里再按通话名单判一次，规则是"谁让我打，我就打给谁"，
/// 不接受任意号码，免得变成骚扰工具。**打完必须确认电话真的响了**：AVSDK 的外呼命令可能被丢弃，
