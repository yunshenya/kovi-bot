...
impl PostgresRelationNoteStore {
    ...
    ///
    /// **这里没有作用域谓词：传进来的每个键都是全局删**，所以调用方只允许传无歧义
    /// 标识（QQ 号 / external_id）。显示名不在其列——谁都能把昵称改成别人的名字，
    /// 传进来就会删掉无关群里关于那位真实成员的结论。代价是模型用昵称写下的结论会
    /// 留下（覆盖不到穷尽，是不对齐身份的既定代价），这比误删他人的数据可接受。
    /// 按群擦除走的是 [`Self::delete_conversations`]，按会话作用域删，不受影响。
