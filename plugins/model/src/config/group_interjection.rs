...
pub struct GroupInterjectionConfig {
    ...
    /// 对"每句话都回"的刷屏波次做硬性控制。
    ///
    /// 点名与未点名**共用**这一份额度（`reserve_visible_reply_slot` 不看身份）。
    /// 管理员豁免的是**等待间隔**（调用方传 `gap=0`）：管理员的话是明确
    /// 指令，不该被"刚刚回过别人"挡在门外；`reply_rate_limit` 频率上限对
    /// 所有人一致，所以豁免的只是等待，不是额度。
    /// （注释原先写着"管理员豁免"却与实现对不上，2026-09-14 更正；当天
    /// 又按实测把间隔豁免补了回来——管理员指令被 20 秒间隔静默丢掉。）
    reply_gap_secs: u64,
