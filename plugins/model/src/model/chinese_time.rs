...
fn resolve_weekday(text: &str, today: NaiveDate) -> Option<(NaiveDate, String)> {
    ...
    // 顺序有讲究：带周次的长标记必须排在裸标记前面。否则"下星期三"会先命中
    // 数组里的"星期"、按本周算——周次被悄悄丢掉，提醒整整错一周。
    for marker in [
        "下个星期",
        "下个礼拜",
        "下星期",
        ...
        let week_offset: i64 = match marker {
            "下个星期" | "下个礼拜" | "下星期" | "下礼拜" | "下周" | "下週" => 1,
            "上个星期" | "上个礼拜" | "上星期" | "上礼拜" | "上周" | "上週" => -1,
            _ => 0,
        };
...
mod tests {
    ...
    #[test]
    fn weekday_qualifiers_apply_to_weekday_forms_too() {
        // "下周三"与"下星期三"是同一个意思，不能因为写法不同就落到不同的周。
        // 周五(2026-09-11)所在周的周一是 09-07。
        assert_eq!(at("下星期三"), "2026-09-16 00:00");
        assert_eq!(at("上星期三"), "2026-09-02 00:00");
        assert_eq!(at("这星期三"), "2026-09-09 00:00");
        assert_eq!(at("本星期三"), "2026-09-09 00:00");
        assert_eq!(at("下星期天"), "2026-09-20 00:00");
        assert_eq!(at("上星期四"), "2026-09-03 00:00");
        // 带"个"的写法同样常见。
        assert_eq!(at("下个星期二"), "2026-09-15 00:00");
        assert_eq!(at("上个星期四"), "2026-09-03 00:00");
        assert_eq!(at("这个星期三"), "2026-09-09 00:00");
        // "礼拜"是"星期"的口语说法，周次同样要生效。
        assert_eq!(at("下礼拜一"), "2026-09-14 00:00");
        assert_eq!(at("上礼拜五"), "2026-09-04 00:00");
        assert_eq!(at("下个礼拜一"), "2026-09-14 00:00");
        // 不带周次时仍然是本周，且过去的日期如实标成过去。
        assert_eq!(at("星期三"), "2026-09-09 00:00");
        assert!(resolve("上星期三", now()).expect("可解析").in_past);
    }
