...
pub(crate) async fn private_message_event_after_ingress(
    ...
    if is_restricted_command(message) && !sender_is_admin {
        // 这一刀切是**规则**，不是疏漏：**命令只由管理员下达**。授权好友能和她聊天、
        // 能打进来，但不能指挥她做事——`#打给我` 和 `#系统信息` 是同一条规则，不是特例。
        // 所以 `#打给我` 的实现里虽然另有一道"通话授权名单"判据，那管的是"谁能打进来"，
        // 不是"谁能下命令"：两道判据语义不同、串起来用，长得像矛盾而已。
        // 真机 2026-09-12：名单里的非管理员发 `#打给我` 被静默，确认过这是期望行为。
