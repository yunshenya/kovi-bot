...
pub(crate) enum ReplyScope {
    ...
    /// 独立于聊天会话的持久化任务执行代数。
    Scheduled(i64),
    /// 一通 QQ 语音通话，参数是通话对端的 QQ 号。
    ///
    /// 电话必须有自己的代数：通话中她可能要跑工具（查资料、发消息、建提醒），
    /// 而工具执行要用 `ReplyTicket` 证明"这一轮仍是当前轮"。若借用
    /// `Private(对方)`，对方在通话期间发一条私聊就会推进那个作用域的代数，
    /// 通话里的工具调用会被判成"已被更新的私聊消息打断"而整批失败。
    Call(i64),
}
