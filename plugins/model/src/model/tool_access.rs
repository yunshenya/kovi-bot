...
pub(crate) async fn initialize() -> Result<()> {
    ...
    definitions.push(ToolDefinition {
        ...
        // 描述刻意短：工具 schema 随**每一个**带它的请求发送，长描述就是常驻开销。
        // 但短不等于可以省掉触发条件——"什么时候该调它"正是该写在这里的东西
        // （AGENTS.md 第 6 条：能写进工具 description 的就别抄进提示词）。
        //
        // 这里曾经写着"清单已在提示词里，需要复核或看全时调用"，那是"清单常驻"那版口径的
        // 残留；用户同日把常驻否掉之后，这句话就变成了反向误导——它等于告诉模型"你不需要
        // 为了发图先查清单"，于是照旧凭印象编标签。
        description: "列出她相册里现在能发的全部表情包标签。清单不在提示词里：想发图时先调它拿到准确标签，标签照抄、别自己起名字。"
            .to_string(),
        input_schema: json!({
            "type": "object",
            "additionalProperties": false
...
///
/// 措辞把它说成**她自己的相册**（产品要求：那些图就是她自己的，带她名字的那张就是
/// 她本人的照片），并且把"清单之外一律没有"写死——线上 2026-09-15 13:21 她就是先
