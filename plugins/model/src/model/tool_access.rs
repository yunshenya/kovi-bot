...
pub(crate) async fn initialize() -> Result<()> {
    ...
    definitions.push(ToolDefinition {
        ...
        // 清单现在**常驻提示词**（见 `sticker_library::prompt_instruction`），所以这个工具
        // 只服务两种情况：清单太长被截断时看全，以及她自己想再确认一次——描述里不再说
        // "要发图先调它"，那正是她"以为要查才知道"、进而凭印象编标签的由来。
        description: "再看一眼她自己相册里现在能发的图（完整标签清单）。清单已在提示词里，需要复核或看全时调用。"
            .to_string(),
        input_schema: json!({
            "type": "object",
            "additionalProperties": false
