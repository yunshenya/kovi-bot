...
查询"我喜欢安静的地方"，候选里放一条**语义正确**的文档（"他讨厌吵闹的环境"）：

| 文档 | bge-reranker-base | mmarco-mMiniLMv2（int8） | mmarco（fp32） |
|---|---|---|---|
| 他讨厌吵闹的环境 | **0.2758 第 1** ✅ | — | — |
| 明天下午三点开会 | 0.00025 | **0.0016 第 1** ❌ | 同样排第 1 ❌ |
| 分数量级 | 0.276 / 0.041 / 0.0002 | 全部塌到 0.001 量级 | 同上 |

三件事：

1. **不是排序边缘问题，是排序错误**——它把完全无关的"明天下午三点开会"排到了第 1，
   真正相关的那条没上来。这比"分不分得开"严重：一个会**主动改坏**融合顺序的重排器，
   比没有重排器更糟。
2. **不是量化的锅**：我用 fp32 权重重跑，排序一模一样地错。所以问题在这个 checkpoint
   的判别力，不在 int8。
3. 时间没白花在别处：它**反证了重排器不是可以随便换的**——Hindsight 用 cross-encoder
   之所以值，是因为那个 cross-encoder 真的判得准；换小了就只剩"多一次推理"的成本。
