...
重排器（bge-reranker-base，量化 279MB）加载后常驻约 800MB。ONNX Runtime 的常驻
远大于模型文件：量化文件 279MB ≠ 进程里 800MB。这台机器加不起。
...
2. **给服务加内存围栏**（这次事故真正的结构性修复）：
   ```ini
   MemoryHigh=512M
   MemoryMax=700M
   ```
   有围栏之后，超限只会杀这一个服务（systemd 自己动手），而不是把整机拖到
   load 55 再让内核挑一个牺牲品。**没有围栏才是这次事故能波及整机的原因。**
3. 配置显式写进 `bot.conf.toml`：原先 `embedding_*` 全走代码默认值，
   我用 `sed` 改配置文件时**一条都没匹配到**——"改默认值改不动钉死的部署"
   这条今天已经踩过一次，这是第二次。

处理后：嵌入服务 400MB、机器可用 2.4GB、load 4.12、回填继续推进、
`NRestarts=0`（围栏没在杀它）。
