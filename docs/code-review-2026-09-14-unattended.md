...
## 六、第二批修复（2026-09-14，全部优化）

按"明显问题顺手修"的标准清掉的一批（每条独立提交）：

| 提交 | 问题 |
| --- | --- |
| `ed03364` | **管理后台写主配置会把运行时覆盖从内存里挤掉**（`write_raw`/`patch`/`restore_backup` 三条路共用同一个校验函数）。主配置只按自己校验，而实际加载是"主配置 → 叠加覆盖"，校验通过后 `install` 的就是那份"主配置单独"的结果——后台改一个字段，连带静默丢掉一组覆盖设置，界面看不出异常。新增 `validate_main_candidate`（= 候选主配置 + 现存覆盖），并把覆盖路径做成参数以便测试。两条测试都实测过"退回旧行为会变红"。 |
| `14360fd` | 模板补齐线上在用却没进模板的 9 个键（`memory.embedding_*` 6 个、`group_interjection.sticker_reaction_*` 3 个）。值与代码默认值相同，行为不变，只为消掉这个方向的漂移。`proactive.main_admin` 刻意不加（真实 QQ 号，由部署工作流注入）。 |
| `6a51667` | `tools/turngate/train.py` 的 train/val 切分写反：`val_all, train_all = perm[n_val:], perm[:n_val]` → 按 `--val-frac 0.2` 就是"20% 训练、80% 标定"，第 329 行打印的两个数还都是 `n_val`。 |
| `940ae34` | `scripts/verify-chat-shape.sh` 取不到日志时所有指标打印 0（含"BUBBLE 泄漏 : 0"）并以 0 退出——把"什么都没检查"读成"确认没有泄漏"，正是它要防的假通过。 |
| `f656357` | `qq_call.caller_allowlist_file` 留空被 validate 无条件拒绝，与它自己的字段文档（"留空关闭这个能力"）和唯一消费方（见到空路径直接返回）矛盾。改成只拦"打开了授权却不给路径"。 |
...
## 十、第六批修复（本轮 8 条里的 6 条）

| 提交 | 条目 | 结果 |
| --- | --- | --- |
| `ebfaf73` | `group_access.rs:196`、`turn_gate_runtime.rs:203` | 授权名单发布串行化 + 锁内取快照（否则后落盘的用更旧快照）；状态命令改报**生效值**并在与配置不一致时说明 |
| `0746327` | `consolidation.rs` | 四个类型都改成按"不早于已存状态"取时间，批次不再因时间戳回退整批丢弃 |
| `4a6a988` | `memory/mod.rs:497` | 池大小与取连接超时改成**可配置**（默认值一个没动，行为不变） |
| `f656357`→ 本轮 | `memory_transport.rs:138` | 解禁时一并清"发送被拒退避"（新增 `send_guard::clear`），`#结束禁言` 不再看着解禁了却继续静默 |
| `aafed9b` | `admin/config_api.rs:250` | 配置接口的 `raw` 也打码，并在整文件写回时把 `MASK` 还原成磁盘真值——只打码不还原会把"泄漏"变成"一次保存毁掉所有密钥" |

**上线**：`aafed9b`，readiness 通过，本次启动以来 WARN/ERROR 为 0。

### 仍未做（2 条）

- ~~`delivery.rs:554`~~ **已修（`25bf4be`）**：合成挪到 `begin_outgoing_commit` 之前。
  代价是被授权/路由拒绝的发送会白合成一次（异常路径，比"用户收不到回复"轻得多）。
  仍未端到端验证（要 TTS 侧车故意慢过租约再跑真实投递）。
- `memory_store.rs:197` **仍未做**，原因与做法：语句爆炸来自 `delete_legacy_copy` 的
  形状——每行都要查一次迁移账本、解析一次兼容作用域，再对
  (候选 id × subject id × context) 逐个 DELETE，一行约 6 次往返、极端可到 2112 次。
  正确的修法是**把作用域解析提到循环外**（按 scope 分组被删记忆），并把兼容删除折叠成
  每个 (scope, context) 一条 `id = ANY($1)` 的集合删除；账本查询同样可以按
  `id = ANY($1)` 批量。这要动删除与缓存失效的事务边界，属于我此前反复在修的隐私路径，
  没有可信验证手段（要么真库上跑出"数万条语句"的对照，要么不碰）——所以留到下一个
  有验证条件的会话做，不在这条会话的末尾硬塞。
- `memory_store.rs:197` 保留期清理在全局排他锁内逐行级联删除（一行约 6 次往返，可放大到
  数万条语句）→ 要分批 + 折叠兼容删除，动的是维护事务边界，同样缺端到端验证手段。

这两条都不是"改不动"，而是改完**我没有可信的验证手段**（需要真实库上的维护事务与投递
时序），按"报错是信息不是障碍、但别拿没验证过的东西上生产"的原则停在这里。
