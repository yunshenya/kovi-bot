...
# 180. Conversation Collision 与 Mind 的边界

Mind v2 不维护 `Thinking / Prepared / Committed` 等发送并发状态，这些属于 V1 ConversationCoordinator。

但新消息到达时，Mind 必须能先更新相关长期/中期状态，例如：`Curiosity resolved`、`OpenQuestion resolved`、`OpenLoop resolved`、`InnerAgenda changed`。随后 Executive 使用最新 `MindSnapshot` 对 PendingOutgoing 重校验。

典型场景：

```text
InnerAgenda: 想知道用户面试结果
Prepared proactive: “你今天面试怎么样？”
New User Message: “我面试过了！”
```

正确流程：

```text
OpenLoop / Curiosity resolve
→ MindSnapshot update
→ Pending proactive Superseded / Rewritten
```

禁止 Mind 已经知道结果后仍发送旧问题。Mind 不负责真实 send commit，也不持有发送锁。
