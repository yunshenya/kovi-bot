...
# 267. Message Collision 作为 World Observation

World Model v4 新增会话并发相关 Observation：
...
当双方消息已经分别进入不可撤销发送边界并近乎同时发生时，`MessageCollisionDetected` 是正常 WorldEvent，不是系统异常。

World Model 可记录 conversation、incoming/outgoing message、delta_ms、current floor、conversation version，以及 pending question 是否已被新消息回答。

但不得因为“撞消息”自动推断用户生气、着急、不耐烦或故意打断。SocialScene 可把 collision 作为短期会话结构信号。

Planner 下一轮可以自然提及“我们刚好撞消息了”，也可以完全不提；不得每次固定输出模板。WorldModelSnapshot 应为 V3 Outgoing Revalidation 提供 Conversation / SocialScene 信息，但 World Model 本身不得执行 Cancel / Send。
