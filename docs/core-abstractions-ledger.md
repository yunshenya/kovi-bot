...
### 1.2.1 判据怎么强制：**照搬仓库已有的两半机制**，别指望文档

"Core 必须对目标或内容有判断的才配表达"这条判据，写在文档里是挡不住人的。仓库里已经有
现成的强制手段，**照搬**即可——它是**两半**，缺一半都不行：

**第一半：无 `_` 兜底的 match（编译器逼你决定）。** 现成范例在宿主的
`plugins/model/src/model/tool_access.rs`（`BuiltinTool::write_scope`）：
...
Core 侧已经有同样的东西：`arbiter.rs:1401` 的 `capability_for` 与 `permits` 都列全了每个
变体、没有 `_`——**加一个动作变体会编译失败，逼着人当场判"它是表达还是作为"**。

**第二半：配对测试，逐项钉住决定（决定错了会红）。** 范例是同一文件里的
`write_scope_policy_requires_explicit_safe_annotations_and_builtin_review`：**每一个工具
一行断言、附一句理由**（"「只影响本人」与「对外可见」必须分开"…）。

**两半分工不同，别混：**
- 无兜底 match 只保证"有人**做了**决定"，不保证决定**对**；
- 配对测试才能抓住"编译得过但归错了档"。
- 两半都过不了"有人把测试改成迎合错误行为"——但那时他必须**把预期写下来**，
  这正是评审能抓住的东西。**这是这个机制的诚实上限**，不要假装它更严。

**Core 现在只有一半**：`capability_for` / `permits` 无兜底（第一半有了），但**没有**一条
逐变体钉住"动作 → 能力 / 授权位"的测试（第二半缺）。补上它应该是第一步的一部分。

**新的检查点放哪：**
- **Core**：动作的**分类点**（"这是表达还是作为？它有哪些属性？"）用无兜底 match，
  注释直接指向本台账 §1.2；
- **宿主**：新工具已有 `write_scope`（无兜底）与 `result_may_carry_foreign_text`
  （**默认不可信**——机制不同，是有意的 fail-safe，不是遗漏）；
- 两边都配一条逐项测试。
...
| **状态**（**待定**，见 §10 第 13 条） | 若走"路 B"则单列：`State` | 今天混在 `StateUpdateProposal` 里 |
...
**这张表的行数取决于 §10 第 13 条**：选"路 A"（承诺泛指"改她自己"）就是五行；
...
- 验证：现有测试全绿 + 新增"回复在读过外人文字后仍可发""只读工具永不被档位拒"
  "ReachOut 受档位约束"三条。
- **同时补上 §1.2.1 缺的那一半**：一条逐变体钉住"动作 → 能力 / 授权位"的配对测试
  （Core 今天只有无兜底 match，没有这条测试）。
- 回滚：这一步不动枚举与 wire 格式。
...
12. **Core 缺的那半强制机制什么时候补**（§1.2.1）：`capability_for`/`permits` 今天有
    无兜底 match、但没有配对测试。我建议**在第一步就补**——现在补最便宜（变体还只有七个），
    等收口成 `{Speak, Act}` 之后再补，那条测试的覆盖面就小得多。
13. **五个还是六个**（§1 那条说明）：把"承诺"放宽成"改她自己"（路 A，五个），
...
> **下表里的 `§` 编号指的是 `docs/yunxi-core-architecture.md` 的章节，不是本台账的章节。**
