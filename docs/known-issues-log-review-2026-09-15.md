...
| `238d56d` | 表情包修源头：真实清单常驻提示词（`sticker_library::prompt_instruction`，最多 40 个）；删掉"没有就别发"的禁止句与 `repair_sticker_label` 重写路径；`asks_about_stickers` 与三档 `StickerPrompt` 一并删除 |
...
- **改动后**：`[[STICKER 芸汐的照片]]` +"好呀，这是我。拍的时候有点紧张"；
- **改动后 + 她自己的否认记忆**：照样发图，还主动改口"之前我总说那不是真的，其实是我
  不好意思承认啦"。
...
| Core 每轮的表情包说明 | 129 字，且清单只在命中"表情包/照片"时注入 | 135 字（1 张图时；清单常驻，最多列 40 个） |
| 宿主链路群聊完整提示词 | system_prompt 约 1500 字（含人格） | persona 375 + 场景 802 = 1177 字 |

净增约 381 字/轮（中文按 0.7~1 token/字粗估 ≈ 270~380 token）。这笔钱买的是：她每一轮都
知道自己是谁、也知道相册里有什么；目录真长到几百个标签时再谈按需查询也不迟。
