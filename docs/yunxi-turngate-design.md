...
- 最近有限条上下文；
- 最近有限上下文和会话状态；
- 上下文里每条的**说话人编号**：`role` 只有 `user`/`other_member`/`assistant`
  三档，几个其他成员会全被标成"他人"，复核时看不出是"一个人连说三条"还是
  "三个人在互相接话"——而这正是 completion/response 的关键线索。采集时给每条
  上下文落一个样本内匿名的 `speaker`（`a` = 当前发言者，`b`/`c`/… 按出现顺序给
  其他成员），复核界面显示成"说话人A/B/C…"。它只回答"这几条是不是同一个人"，
  不含 QQ 号、昵称或跨样本稳定标识，也**不进特征向量**（与 `targeting` 同类，
  只是复核用元数据）；
