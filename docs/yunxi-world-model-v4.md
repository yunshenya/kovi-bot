...
>
> 📌 **实施路线**：按价值密度排序的渐进计划见 [yunxi-world-model-v4-rollout.md](./yunxi-world-model-v4-rollout.md)。
...
| **Causal / Prediction / Simulation**（§39–52、§53–67） | ✅ **Prediction 领域层 + 插件确定性预测器已实现**（量化概率 0.05 步长 / 不确定绑带 v4 §82 / 校准信号 v4 §52 / 短命 TTL；工具失败 → 环境降级 + RetryNow/Wait/Fallback 影子预测，v4 §102/§196）；✅ **Simulation 领域层已实现**（纯值快照输入、Simulated 模式强制、预算 1–3/根、5 分钟缓存、版本陈旧检测 v4 §67）；✅ **Causal 领域层已实现**（工具/主机/环境模式、重复证据≥3 晋升 v4 §98、person-specific 需显式域规则 v4 §99、快照检索上限 8）；执行侧模拟器接入 ⬜ | `crates/yunxi-core/src/world_model/{prediction.rs,simulation.rs,causal.rs}`；`plugins/model/src/yunxi/world_model.rs::{tool_recovery_outcome,record_tool_failure}` |
...
| **WorldModel Store Ports / Persistence / 表结构**（§125–131，含 `yunxi_world_observations`） | ✅ 已实现（进程内 + Postgres 持久化：`yunxi_world_observations/entities/situations/hypotheses/scenes/uncertainties` + `meta` 版本；advisory-lock 迁移、重启恢复、TTL 清理、`#删除我的数据`/群删除联动） | `plugins/model/src/yunxi/world_model_store.rs`；`crates/yunxi-core/src/world_model/` 的 `restore_from_parts/restore/from_entities/prune_expired` |
| 与 OpenLoop/Mind 的接入 | 🔶 部分（watch 开环已接；§93–95、§112 其余未接） | `observe_world_fact` 的 `world_loop_draft` 分支 |
| **Phase 0–16 建设计划**（§175–193） | 🔶 Phase 0–8 领域层已完成（`cargo test -p yunxi-core` 全绿，零平台耦合，无行为变更）；R1 传感器泛化（file_state）已接入；R2 观测面 `#情境/#world-status` 已可查当前局面；R3 场景喂入+碰撞已接；**R4 行为级接入已就绪（`influence_mode=active` 时用世界 interruption cost 抑制未点名群聊插话，默认 disabled 零行为变更）**；R5 预测领域层+工具失败预测器+**结果回比对校准（Predict→Act→Observe→Compare 闭环）**；R6 模拟领域层+**执行侧工具恢复模拟器**（纯值/Simulated/预算/缓存/陈旧检测）；R7 因果领域层+**工具失败重复≥3 → ObservedRepeatedPattern 晋升**（person-specific 仍禁）；R8 持久化已实现 | `crates/yunxi-core/src/world_model/`；`plugins/model/src/{config/world_model.rs,yunxi/world_model.rs,yunxi/world_model_store.rs,model/world_commands.rs}` |

> **一句话现状**：v4 的**平台无关领域层（Phase 0–8）已全部落地**（有界、构造即校验、280+ 单元测试），插件侧已有**影子模式运行时 + Postgres 持久化**（观察/情境/场景/碰撞喂入、重启恢复、删除联动、`#情境` 观测命令，`[world_model] enabled=false` 默认关闭，绝不阻塞聊天）；**R4 Executive 行为级软信号仍为日志影子状态**，预测/模拟（R5–R7）仍在 rollout 路线图中。下文设计节引用的表结构已在实现中按目标 schema 建表。
