...
**2026-09-15 修订（这条约束不再写进提示词）**：上面这句曾以自我认知的形式常驻每一轮
提示词（`SelfIdentity` 的 description 加 `ai_driven` / `claims_human_identity` /
`host_independent` 三个由 `validate` 强制维持的标记），结果是它与人设直接打架——有人
要她的照片时她答"我哪有什么照片呀，就是个只会打字陪你聊天的人"，连素材库里那张
"芸汐的照片"都不肯认（线上 2026-09-15 13:20）。

现在的分工是：**她是誰由配置里唯一一份 `prompt.persona` 负责**，Core 与宿主两条链路
共用；自我认知只剩名字与一句自我介绍，不再声明技术身份，也不再由代码强制"必须承认
自己是 AI"。这条设计原则本身没被推翻——她仍然不是人类、也不该声称有肉身——只是不再
用一句技术声明去表达它：那句话说给模型听时，压掉的是人设而不是幻觉。
...
| 9 Relation | 部分完成 | Relation 绑定 `PersonId`；**五个维度各有唯一写者**——`familiarity`/`comfort` 由回合的结构演化写（递减步幅、按会话类型），`tension`/`affinity`/`trust` 由相处证据的 delta 通道写（`relation_evidence_nudge` 把一条有符号证据同时折成短期冷暖与长期好感/信任，锚点与半衰期都在 Core）。语义 cues 只改情绪，不改关系：回合收尾回写的是回合开始时的快照，它一旦也写那三列就会覆盖掉回合期间到账的证据。`affinity` 驱动语气档与群聊未点名放行，`tension` 驱动语气档与静默门控。PostgreSQL 读取时应用确定性时间衰减（`relation_half_lives` 是唯一来源），每个写者的 UPDATE 都把别列的漂移一起落库；legacy profile 只填充缺失行且不再给新用户负好感。错误 Person 的已加载状态不会传播；读取失败（而非"没有行"）时本轮不回写。跨天、跨重启和跨平台人工验收仍待完成。 |
...
plan；这是 Host adapter，不是 Core 对旧模型模块的依赖。当前 Core 接管私聊安全纯文本/普通
图片以及群聊普通文本/普通图片；未点名群聊采用本地抽样后由 Core 决定回复或沉默。命令、
