...
### 11.16 上游中途静默不再白等 60 秒：SSE 流加 20 秒空闲判据（`5216758`）

11.15 修的是"已经生成好的回复被丢掉"，这一轮修"卡住"的另一半：**上游接了请求、回了响应头，
然后一个字节都不再发**。

线上形态（11.15 里那两次 `status=200 terminal=parse_error`）：错误是
`模型响应读取失败: error decoding response body`，各把 `server.request_timeout_secs`
（60 秒）耗满才重试，而重试只用 1.0 秒 / 11.9 秒就成功。整请求超时是唯一一道闸，它对
"完全不发数据"和"正在慢慢生成"一视同仁，所以只能等到最后。

**改法**：给 SSE 流加空闲判据 `MODEL_STREAM_IDLE_TIMEOUT = 20s`，由
`stream_idle_timeout(request_timeout)` 取 min（配置把整请求超时压得更短时以配置为准，
两条闸不互相矛盾）。`read_model_payload` 的读取循环改成：event-stream 的每次 `chunk()`
都套这个判据，超时报"模型流式响应空闲超过 N 秒（上游在响应中途停止发送数据）"，上层照原样
立刻重试。第一次 `chunk()` 就是同一个等待，所以首字节也一并管住，不必再单设 TTFB 预算。
**非流式响应不套它**：那种响应的正文是生成结束时一次性到达的，中间本来就一个字节都没有，
拿空闲判据卡它会误杀正常的长生成（仍然只受整请求超时约束）。

20 秒的依据是实测：当天**成功**的调用里最长 11.9 秒（还是重试那一趟），只有两次超过 5 秒；
SSE 生成期间每个 delta 都是一次读取，正常不会出现 20 秒静默。最坏情况也从
"3 次尝试 × 60 秒 = 180 秒"降到 60 秒。

**验证**：`the_stream_idle_timeout_stays_below_the_request_timeout`（60 秒请求 → 20 秒判据；
5 秒请求 → 5 秒判据）；`a_silent_upstream_is_cut_off_by_the_stream_idle_timeout`
起一个本地 SSE 服务、回完响应头与一个 delta 就闭嘴（比判据长得多），用真正的
`read_model_payload` 断言必须在空闲判据内失败、错误文案说明是空闲超时——跑的是真 HTTP +
真 SSE 解析，0.31 秒完成。fmt、clippy 干净；model 1167、yunxi-core 355、yunxi-cli 10、
acceptance 13 全绿。

**已上线**（`5216758`）：推 `5fdd52f..5216758` → 演练（包 14.2 MiB、sha256 `c752e472…`）
→ 产物自检（空闲判据文案与上一轮的重试日志都在）→ 真部署（`--no-build`）→ 服务 active、
重启 0 次、WARN/ERROR 0、工具注册表就绪、QQ 已连。

### 11.17 认知循环的归属迁移上线：循环、工作记忆、预期、预算、目标、自我效力都进了 Core（`5eeff5b`）

前面几节修的是回复链路的具体毛病；这一轮改的是**结构**：把"认知循环"从每个宿主各写一份，
搬成 Core 自己拥有。判决与理由单独成文——
[`yunxi-core-boundary.md`](yunxi-core-boundary.md)（边界决策记录），完整改动是
`81a988e..5eeff5b` 共 24 个提交，这里只记上线与**对线上行为的影响**。

**改了什么**：新增 `crates/yunxi-core/src/driver.rs`（`run`/`drain` + `CognitiveTurnObserver`）、
`working_memory.rs`（跨回合工作记忆）、`TaskBudget`（按轮的任务预算）、`PlanGoal`（任务目标）、
`EffectScope` + 每轮效果档位、预期的推导与声明两条生产者（跨任务预期归**会话**）、自我效力
证据 → 自我模型局限。Kovi 的 `run_runtime` 从 283 行降到 20 行，只剩端口装配与宿主记账。

**对线上行为的影响（运维要知道的）**：

1. **`tools.max_rounds` 现在真的生效**。此前它只作用于遗留路径，回复路径实际跑 Core 硬编码的
   默认 8；生产配置写的是 `max_rounds = 10`，接上之后**有效轮数从 8 变成 10**（兑现配置意图）。
   想回到 8 就在配置里写 `8`。
2. **未声明的工具会被拒**。宿主按自己的注册表声明工具（含 MCP），所以它自己的工具照常；但任何
   注册表里没有的工具名（模型幻觉）现在 fail-closed，而不再是一路放行到宿主。
3. **效果档位收窄会改变下发的工具集**。吃过"可能含外人文字"的工具结果（`web.search`、
   `memory.search`、`weather.*`、MCP…）或上一步失败之后，该任务收窄到 `UserScoped`：仍可读、
   仍可改她自己本人的状态，但不能对外发言。注意这比旧口径**更持久**——旧的是"看紧邻那一条
   结果"，新的是"这个任务吃过就一直算"。跨群问答（`group.message.targets` → `send`）不受影响，
   因为 targets 按宿主数据声明为不含外人文字。
4. **提示词多了两处**：工具轮会带 `task.declare` 工具（让她声明目标与"之后应当发生什么"）；
   persona floor 多一句她自己的局限。工具结果跟进轮会带"任务目标"与"工具 X 已连续失败 3 次"。
5. **自我模型会学新的局限**（连续 3 件没做成 → 巩固时写入），并且局限**第一次被渲染进提示词**。

**部署前专门查过的两条生产风险**（都不是"应该没事"，是查过）：Core 路径不下发
`reply_action`（`allow_reply_actions: false`，否则它会被新的 fail-closed 闸门拒掉）；
工具注册表在 bridge 之前 `await` 初始化，且 MCP 在 `initialize()` 内连接，所以启动快照
**包含** MCP 工具，不会因为声明为空而把所有调用拒掉。

**验证**：`cargo test --workspace` 全绿（model 1175、core 395、CLI 14），clippy 无告警；
关键修复做过变异验证（把实现改回错误版本确认测试变红）。

**已上线**（`5eeff5b`）：推 `39c9497..5eeff5b`（会话前半段的提交此前已在远端）→ 演练
（包 14.2 MiB、sha256 `d2d347bb…`）→ 真部署（sha256 `6b5c47f5…`）→ 服务 active、重启 0 次、
WARN/ERROR 0、readiness 已写、巩固流程正常跑过（`Yunxi Mind reflection … trigger=Maintenance`）。
回滚目标是 `5216758`。
