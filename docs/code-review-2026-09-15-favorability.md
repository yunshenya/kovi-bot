...
1. **F10 legacy 画像只在 Host 群聊路更新**：私聊与被 @ 的消息都不再学
   `relationship_level` / `interests` / `mood_history`。修法涉及热路径（`group.rs` /
   `private.rs` / `utils.rs`）与"每个已理解的消息学一次、且只学一次"的计数语义，
   需要真实流量验证，建议单独一轮。
2. **W5 场景/宿主/工具上限只拒不留**：超过上限后新的会话/工具记不进来，靠"删除路径"
   兜底而那条路径没有生产调用点。修法要定"淘汰谁"的语义。
