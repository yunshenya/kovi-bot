...
> **谁能让她打出去**：只有主管理员与副管理员。命令在私聊分发层属于"受限命令"，
> 非管理员发的会被静默丢弃——通话授权名单里的非管理员也不行（2026-09-12 真机上
> 就这么静默了一次，本人完全不知道发生了什么）。这是有意的：**通话名单管"谁能打进来"，
> 外呼是给管理员的动作**。两道判据看着矛盾（实现里查的是通话名单），实际是"先管理员、
> 再名单"的串联，后者对管理员恒真。
