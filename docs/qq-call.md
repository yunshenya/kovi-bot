...
- **授权只决定"她接起来之后说不说话"，不决定"接不接"**：接听由桥完成，插件拦不住
  ...
  "来电振铃"那一行区分（见[排错](#排错)）。
- 未授权的人**根本不会被接通**：机器人把「授权名单 ∪ 副管理员 ∪ 主管理员」写进
  `caller_allowlist_file`，桥在接听前读它，名单外直接不接，来电自然结束（对方听到
  "无人接听"）。这是 2026-09-12 加的：QQ 的 1v1 通话没有对插件开放"离开房间"，
  "接通后婉拒"会把通话留在 `connected`，正是 9 月 11 日晚上卡死一整夜的成因。
- 名单文件读不到时（例如机器人没在跑）桥会**回退成接听任何来电**，避免谁都打不进来；
  `#通话状态` 会显示名单人数与最近同步时间。
...
1. **未授权来电不接**：机器人把「授权名单 ∪ 副管理员 ∪ 主管理员」写进
   `qq_call.caller_allowlist_file`（默认 `/home/ubuntu/napcat-qq-call/bridge/runtime/allowed-callers.json`），
   桥在接听前读它，名单外直接不接听——上面那次事故的成因从根上消失。
