...
**不通话也能验：`#通话自检 [问题]`**（授权来电者可用，别名 `#电话自检`）。它复用
**同一个** `generate_reply` 跑一遍工具链路——同样的模型调用、同样的工具清单、同样的
结果回灌——只换两样：音频出口换成记录（不出声），工具换成只读（走
`ToolRegistry::execute_read_only`，**执行层硬拦**副作用，所以它永远不可能真的发出
消息或建提醒）。报告会说清：可用几个工具、调了哪些、成功没有、本来会说什么、
以及工具慢到会不会触发填充语。

存在的理由：验证"电话里能不能办事"原本必须真打一通电话，成本高、还得有人正好有空接。
`#通话自检 现在几点` 在私聊里发一句就能把除音频与桥以外的整条链路验穿。
