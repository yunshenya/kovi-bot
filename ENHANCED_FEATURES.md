...
### 3. 话题生成器 💬
- **10种话题类型**: 日常、深度、趣味、个人、时事、创意、怀旧、未来、知识和社交
- **个性化话题**: 根据用户兴趣生成专属话题
- **情绪适配**: 根据当前情绪选择合适的话题
- **智能话题选择**: 根据时间适配表达，并在冷却期内避免重复
...
### 核心特性
- **异步处理**: 模型调用和记忆持久化均异步执行，不会用文件 I/O 阻塞运行时线程
- **线程安全**: 使用Arc和Mutex确保多线程安全
- **错误处理**: 完善的错误处理机制
- **可配置**: 主动消息、记忆容量与保留期、情绪缓存与漂移、话题冷却均可通过配置文件调整
...
2. **主动聊天**: 只会选择近期活跃群组，以及曾经真正私聊过的用户
3. **性能优化**: 后台维护任务会自动去重、限量并清理旧记忆
