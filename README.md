...
样本详情排到队列前面、统计与队列分布折进一行可展开的标题。宽屏与平板的版式不受影响。
...
- **配置**：展示 `bot.conf.toml` 的**全部参数**（含没写进文件的默认值，带"默认值"标记）以及
  ...
  保存时按字段写回并**保留注释**，每次改动前自动备份、可一键回滚。分区说明和字段说明由
...
在后台的「表情包」页可以**直接上传/删除**素材（标签在页面上填，同名自动加编号），也可以像上面那样往目录里丢文件——两条路写的是同一批文件，列表里看到的就是她能发的。目录**不需要你手动创建**（与 `admin.annotation_dir` 同一约定）：启动时缺了就补建，运行中被删掉也会在下次重扫时长回来；补建不出来（权限、只读文件系统、路径被同名文件占住）时启动日志里会有一行 `[ERROR] 表情包素材库目录不可用`，不会闷着不说。往目录里加/删素材后最多等 `rescan_secs`（默认 30 秒）生效，不必重启；素材数量变化时日志里会有一行 `表情包素材库已加载 N 张图 / M 个标签`（只在变化时打，不会按重扫周期刷屏）。她有两个出口：

- **自己判断该不该发**：Core 的可见回复契约里多了一个前导标记 `[[STICKER 标签]]`（与 `[[VOICE]]`、`[[SING 模板id]]` 并列）。写 `[[STICKER 无语又想笑]] 我也是这么想的。` 就是把那张图贴在消息里一起发；正文可以留空，那就只发一张表情——这仍然算一条正常的可见回复，不是沉默。提示词里只会列出素材库真实存在的标签，素材库空了或 `enabled = false` 时这个出口根本不下发。
