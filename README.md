...
群聊可见回复还受确定性节奏硬限制兜底，与模型判断无关：同群两次可见聊天回复（点名或未点名）至少间隔 `reply_gap_secs`（默认 90 秒），且每 `reply_rate_window_secs`（默认 600 秒）内同一群最多输出 `reply_rate_limit`（默认 4）条可见回复。管理员与普通成员同等受限（控制面命令、显式识图/表情教学、以及用户明确要求的多条消息不受此限制）。无论模型多想接话、接续窗口多热，这个上限都会把回复频率压住，防止“几乎每句话她都回复”的刷屏。Host 与 Core 共用同一预算。
