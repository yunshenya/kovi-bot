...
- **标注**：TurnGate 待复核样本的网页端（设计文档 §7.4 B）。左边是按"标注价值"排的队列
  （tier / 上下文条数 / 排序理由），右边是单条详情——当前句、前一句、最近几轮（按**说话人**
  着色：芸汐 / 说话人A / 说话人B、C…）、弱标签与来源；点按钮或按快捷键打标（`1/2/3` 选
  ...
  上下文里的说话人编号来自采集器（`tools/turngate/collector.py`）：`a` 恒为当前发言者
  （正文与"前一句"都属于他），`b`/`c`/… 按出现顺序给同一段上下文里的其他成员，每人一色。
  编号是**样本内匿名**的，只回答"这几条是不是同一个人"，不含 QQ 号或昵称——没有它，
  几个其他人都显示成"他人"，标注时看不出是"一个人连说三条"还是"三个人在互相接话"。
  采于该字段之前的批次（2026-09-13 前）没有说话人编号，页面会明说"说话人未记录"并退回
  按角色显示。
