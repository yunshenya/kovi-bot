...
在后台的「表情包」页可以**直接上传/删除**素材（标签在页面上填，同名自动加编号），也可以像上面那样往目录里丢文件——两条路写的是同一批文件，列表里看到的就是她能发的。关于开销：一份 60 个标签的清单加说明如果常驻提示词，大约是 400 token/轮（每个可见回合都付一次，而发表情包一天也就几次），所以清单改成了按需查询。想确认她这一刻能发什么，管理员发 `#表情列表` 即可。
...
- **自己判断该不该发**：回复契约里有一个前导标记 `[[STICKER 标签]]`（与 `[[VOICE]]`、`[[SING 模板id]]` 并列）。写 `[[STICKER 无语又想笑]] 我也是这么想的。` 就是把那张图贴在消息里一起发；正文可以留空，那就只发一张表情——这仍然算一条正常的可见回复，不是沉默。**标签清单不常驻提示词**：她真要发的时候先调用只读工具 `sticker.list` 拿一次清单（常驻提示词里只有一句话，工具 schema 也只在素材库非空时才随请求下发），拿到标签就在紧接着的那一轮把标记写出来。素材库空了或 `enabled = false` 时这个出口和这个工具都不下发。
- **管理员命令兜底**：在群里或私聊发 `#表情列表` 看有哪些标签，发 `#发表情 无语又想笑` 让她直接发一张。这条路径不经过模型，是验收"QQ 里真的发得出去"最直接的方式；素材取不到或发送失败时会如实回一句文字，绝不假装发过。

**这批图在她那里是"她自己的相册"**：标签就是文件名，带她自己名字的标签（例如 `芸汐的照片`）她会当作本人的照片——有人要看她的照片，她直接发那张，而不是否认"那不是我真人的样子"。相册语义跟着表情包协议一起进提示词（素材库为空或 `enabled = false` 时不下发）；因为"要她的照片"和"要表情包"在她那里是同一类请求，问句里出现 `照片` / `相册` / `自拍` 也会触发一次标签清单注入。清单之外的东西一律没有：协议里写死了不许先答应再找图，她也拿不到清单里没有的标签。
