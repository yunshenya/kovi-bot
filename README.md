...
她成功回复后为当时的聊天成员开启滚动窗口（`continuation_window_secs`，默认 60 秒），不再依赖接续词表：窗口参与者发出的消息由语义理解判断是否自然接着当前话题，符合时才把窗口截止时间向后延长；她刚发言后的 45 秒内，其他成员也可以直接接上并加入窗口。窗口取值必须 ≤ `reply_gap_secs`——窗口比"她下次可以发言"的等待还长时，它会变成常开状态，低频抽样与候选冷却全部作废；超限时启动会告警并按回复间隔收口，不拒绝启动。窗口内的语义评估同样占用 `decision_rate_limit` 额度，不会因为"在窗口里"就绕过频率上限。

未点名的群聊回合只有在 Mind 真的提出了带由头的开口（接得上某个高显著度的 open question / agenda / interest）时才会有可见输出；Mind 判定 silent 时一律保持观察（`ambient_requires_mind_intent`，默认开启）。关闭该开关即回到旧行为：silent 只作为提示，接不接由模型自行决定。线上 2026-09-10~13 实测，旧行为下未点名回合里 Mind 判 silent 却仍发出可见回复的有 271 条，占全部群回复的 75%。

群聊可见回复还受确定性节奏硬限制兜底，与模型判断无关：同群两次可见聊天回复至少间隔 `reply_gap_secs`（默认 90 秒），且每 `reply_rate_window_secs`（默认 600 秒）内同一群最多输出 `reply_rate_limit`（默认 4）条可见回复。被明确点名或被引用的消息走更短的 `addressed_reply_gap_secs`（默认 20 秒）：有人直接问她问题时，"刚回过别人"不该让问题被静默丢掉；放松的只是等待时长，频率上限对两条通道一致。管理员与普通成员同等受限（控制面命令、显式识图/表情教学、以及用户明确要求的多条消息不受此限制）。无论模型多想接话、接续窗口多热，这个上限都会把回复频率压住，防止"几乎每句话她都回复"的刷屏。Host 与 Core 共用同一预算；预算只记录真正拿到名额的回复——被拒的尝试不写时间戳，否则一次被拒会把"上次回复"顶到当前时刻，让紧随其后的提问再被压一个间隔。
...
直接 `@`、回复或文字点名还带有按“群 + 成员”隔离的防刷限制：不比较消息文本，只按 60 秒内的触发次数计数，超过 4 次进入 10 分钟冷却。管理员与普通成员同等受限。普通模型只生成可见自然正文；是否发送、气泡数量、顺序和连续回合都由宿主状态机编排。每个气泡都应承载新的、无法自然合并的信息，详细解释、步骤、复杂分析或不能过度缩短的安慰仍可按内容需要完整回答，不会被固定字数截断。

普通聊天是否发送由入站语义、权限和宿主状态机决定；模型正文为空或包含内部标记时会被拒绝并进入有界修复，不会把模型自行构造的 silent/JSON 当作控制信号。引用与 `@` 走回复动作工具（`reply_action`）的独立受控分支，撤回走注册表工具（`message.recall`），都不会混入普通正文。
...
- **模型档案**：存过的几套端点各是一张卡片（首字方块 + 名字 + 协议/模型/输出上限 + 地址，
  外加「正在用 / 带密钥 / 无密钥 / 能读图 / 免鉴权」这些标记）。**点卡片任意位置就切过去**，
  立刻生效、不需要重启；「编辑」把那一套填进下面的表单，「保存并应用」按 `profile_id`
  就地更新（不再只能"再存一套同名的"），密钥留空表示沿用这套档案里的那把。表单默认折起，
  由「+ 新增档案」展开。档案存在运行时目录的 `model_profiles.toml`。
- **正在用的那套不能删**：删除按钮是灰的，直接打 `DELETE /api/model/profiles/{id}` 也会拿到
  409 与一句说明。判定分两步：先按"外部模型已启用 且 地址与模型名跟当前 `[server_config]`
  一致"挑出候选（密钥、输出上限、协议是可变量，不参与判定），再用档案文件里记着的
  「最后应用过哪套」（`applied_profile_id`）在候选里点出真正在跑的那一个——所以从当前配置
  "再存一套"出来的副本只是副本，删得掉。这份记录缺失或已经对不上时（比如配置被手工改到了
  别处）退回保守口径：候选全都算，宁可多保护一套，也不放行删掉真正在跑的那套。要删它就先
  切到另一套档案，或在配置页关掉外部模型（`server_config.enabled = false`，那时她只用本地
  能力，谁也不在跑）——档案里存的是这套端点的模板与密钥，删掉就只剩配置里那一份了。
...
素材目录里每个图片文件就是一张表情包，**文件名（去掉扩展名）就是标签**；同一标签想放多张就加编号（`开心-1.png`、`开心-2.png` 都归到「开心」，发的时候轮换）：

```text
/home/ubuntu/yunxi-stickers/
├── 无语又想笑.gif
├── 开心-1.png
├── 开心-2.png
└── 收到.webp
...
[qq_sticker]
enabled = true
dir = "/home/ubuntu/yunxi-stickers"
```

在后台的「表情包」页可以**直接上传/删除**素材（标签在页面上填，同名自动加编号），也可以像上面那样往目录里丢文件——两条路写的是同一批文件，列表里看到的就是她能发的。标签清单**不进提示词**（素材一多，每轮都带上那些标签就是白花钱，而它们只在真要发图的那几轮才有用）：她真要发的时候自己调一次只读工具 `sticker.list` 拿清单，拿到标签就在紧接着的那一轮把标记写出来。想确认她这一刻能发什么，管理员发 `#表情列表` 即可。

目录**不需要你手动创建**（与 `admin.annotation_dir` 同一约定）：启动时缺了就补建，运行中被删掉也会在下次重扫时长回来；补建不出来（权限、只读文件系统、路径被同名文件占住）时启动日志里会有一行 `[ERROR] 表情包素材库目录不可用`，不会闷着不说。往目录里加/删素材后最多等 `rescan_secs`（默认 30 秒）生效，不必重启；素材数量变化时日志里会有一行 `表情包素材库已加载 N 张图 / M 个标签`（只在变化时打，不会按重扫周期刷屏）。她有两个出口：

- **自己判断该不该发**：回复契约里有一个前导标记 `[[STICKER 标签]]`（与 `[[VOICE]]`、`[[SING 模板id]]` 并列）。写 `[[STICKER 无语又想笑]] 我也是这么想的。` 就是把那张图贴在消息里一起发；正文可以留空，那就只发一张表情——这仍然算一条正常的可见回复，不是沉默。**标签清单只在工具里**：提示词里只有一句短协议（"要发就先调 `sticker.list` 拿标签"），清单由工具在那一刻给全。素材库空了或 `enabled = false` 时这个出口和这个工具都不下发。
- **管理员命令兜底**：在群里或私聊发 `#表情列表` 看有哪些标签，发 `#发表情 无语又想笑` 让她直接发一张。这条路径不经过模型，是验收"QQ 里真的发得出去"最直接的方式；素材取不到或发送失败时会如实回一句文字，绝不假装发过。

**这批图在她那里是"她自己的相册"**：标签就是文件名，带她自己名字的标签（例如 `芸汐的照片`）她会当作本人的照片——有人要看她的照片，她直接发那张，而不是否认"那不是我真人的样子"。相册语义跟着表情包说明一起进提示词（素材库为空或 `enabled = false` 时不下发）。她写错标签时宿主**不再拦**：交付层会把取不到的图去掉、正文照发，同时留一行日志——按产品口径，这里修的是"她不知道有什么"（工具随时可查），而不是事后把她的回复丢掉重写一遍。

发送形态是 OneBot 的 `image` 段，`file` 用 `base64://`：不需要 NapCat 与机器人共享文件系统，也不依赖 `qq_voice` 的 `staging_dir` / `napcat_staging_dir` 路径映射。**不发商城表情（`mface`）**：它的 `key` 由服务端下发并绑定具体资源，重发会被 QQ 拒；NapCat 收到商城表情时本来也已经转成 `image` 段，所以以图片段发出去在聊天里看到的效果一致，只是不算"商城表情"那条 UI。素材内容会按文件头校验，把 `.txt` 改名成 `.png` 这种会被当场拒绝并记一行告警。

素材只在投递那一刻从本机读一次，不进模型上下文、也不写入任何数据库；`max_file_kb` 压住的是 WebSocket 单帧体积（整张图 base64 后走 OneBot），不是磁盘占用。
...
- Kovi 管理员：`#授权通话 QQ号`、`#取消授权通话 QQ号`、`#通话名单`；管理谁可以给芸汐打 QQ 语音电话。主管理员与副管理员默认可以通话，其余人必须显式授权；未授权的人打进来只会听到一句婉拒
- Kovi 管理员（私聊）：`#通话状态`（或 `#通话诊断`），一行行列出 QQ 通话是否启用、桥能不能读到、当前阶段与来电者、最近一次通话有没有进房、语音服务是否正常；接听由服务器上的桥负责，这份自检报告用来回答"为什么她没接电话"
- 主管理员：`#授权好友 QQ号`、`#取消授权好友 QQ号`、`#好友名单`；管理谁可以私聊芸汐。授权后对方即可私聊，主管理员与副管理员默认可以
...
私聊授权（`#授权好友`）同样限定主管理员：私聊等于进入芸汐的私人对话，她会带着长期记忆说话，
门槛比群准入更高。通话授权（`#授权通话`）与群准入同级，Kovi 管理员即可执行。
两类名单都保存在 PostgreSQL；数据库不可用时，通话授权会回退到静态配置
`qq_call.allowed_callers`，不会出现“谁都打不进来”。
...
最后一条是**真机探针**：它用配置里的主模型跑两个真实请求，确认那个模型在拿到
`reply_action` 工具声明后（a）明确的 @/引用要求会走工具、参数能过宿主校验，
（b）普通闲聊**不会**去调工具、并且仍然留下可见正文。这是整条回复动作协议唯一无法用
模型替身回答的问题（替身返回的是"provider 会返回的东西"）。它每次都会真的花钱，
所以默认 ignored；改提示词、改工具 schema、或换主模型之后值得手动跑一次。
