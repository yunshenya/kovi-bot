...
- **正在用的那套不能删**：删除按钮是灰的，直接打 `DELETE /api/model/profiles/{id}` 也会拿到
  409 与一句说明。判定口径是"外部模型已启用 且 地址与模型名跟当前 `[server_config]` 一致"
  （密钥、输出上限、协议是可变量，不参与判定）。要删它就先切到另一套档案，或在配置页关掉
  外部模型（`server_config.enabled = false`，那时她只用本地能力，谁也不在跑）——档案里存的
  是这套端点的模板与密钥，删掉就只剩配置里那一份了。
