...
在后台的「表情包」页可以**直接上传/删除**素材（标签在页面上填，同名自动加编号），也可以像上面那样往目录里丢文件——两条路写的是同一批文件，列表里看到的就是她能发的。标签清单**常驻提示词**（最多列 40 个，超出只报个数）：她每一轮都知道自己有哪些图，不会凭印象编一个不存在的标签。想确认她这一刻能发什么，管理员发 `#表情列表` 即可。
...
- **自己判断该不该发**：回复契约里有一个前导标记 `[[STICKER 标签]]`（与 `[[VOICE]]`、`[[SING 模板id]]` 并列）。写 `[[STICKER 无语又想笑]] 我也是这么想的。` 就是把那张图贴在消息里一起发；正文可以留空，那就只发一张表情——这仍然算一条正常的可见回复，不是沉默。**标签清单就在提示词里**（最多 40 个，超过的部分只报个数）：她不需要「先查一次」才能发，可用标签一直看得见——凭印象编标签正是从「看不见清单」来的（线上 2026-09-15 02:15 编了张「猫猫歪头」，13:21 又答应发一张相册里没有的图）。只读工具 `sticker.list` 保留，用于清单被截断时看全或她自己想复核。素材库空了或 `enabled = false` 时这个出口、这份清单和这个工具都不下发。
- **管理员命令兜底**：在群里或私聊发 `#表情列表` 看有哪些标签，发 `#发表情 无语又想笑` 让她直接发一张。这条路径不经过模型，是验收"QQ 里真的发得出去"最直接的方式；素材取不到或发送失败时会如实回一句文字，绝不假装发过。

**这批图在她那里是"她自己的相册"**：标签就是文件名，带她自己名字的标签（例如 `芸汐的照片`）她会当作本人的照片——有人要看她的照片，她直接发那张，而不是否认"那不是我真人的样子"。相册语义跟着表情包说明一起进提示词（素材库为空或 `enabled = false` 时不下发）。她写错标签时宿主**不再拦**：交付层会把取不到的图去掉、正文照发，同时留一行日志——按产品口径，这里修的是源头（让她一直看得见真实清单），而不是事后把她的回复丢掉重写一遍。
