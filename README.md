...
群聊中，普通消息用于学习群组活跃度、成员、话题、氛围和长期上下文；机器人仅在被 `@` 或消息以“芸汐/云汐”开头时调用模型回复。群聊与私聊回复前，模型都会判断是否值得自然延续：通常只发一条，想继续表达时可自行连续发送任意条。每条之间的停顿会随当前情绪、能量、社交信心和随机浮动变化；这不会改变群聊的触发条件。私聊还会持续更新用户的兴趣、性格、关系等级和情绪历史。
