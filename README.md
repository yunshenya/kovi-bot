...
私聊安全纯文本/普通图片和群聊中明确 `@` 芸汐的纯文本/普通图片默认由 Yunxi Core 接管；命令、提醒、Agent
Run、音视频/文件、表情协议和群聊环境消息继续由成熟的 Kovi Host/Adapter 处理。Core 是支持事件的唯一可见回复
owner，Core 入队失败时不会回退到 Host，避免同一消息产生两条回复。
