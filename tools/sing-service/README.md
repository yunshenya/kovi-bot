...
- **拼接一律走交叠**：字内"清辅音 → 变调元音"3 毫秒、音与音之间 15 毫秒（连音）。硬拼接会在
  每个字留一次波形跳变（实测跳变密度是正常说话的两倍），听感就是一路咔哒；逐音淡到零再淡
  起来又会变成一顿一顿的念白。
- 逐音响度对齐只在 ±2.5dB 内调整，整体峰值 0.80：压得太狠会失去起伏，留得太冲会刺耳。
- **整体移调（`transpose`，默认 -5 半音）与 EQ 是"像不像她"的关键**：旋律中心要落在她说话的
  基频（约 250Hz），再用 250Hz 低架 +4dB、3kHz −4.5dB、7kHz 以上 −5dB 把胸腔补回来、硬度压下去。
- **混响默认关闭**（试听后定稿：干声更贴她的音色）；要开用 `--reverb`，注意混响冲激必须先低通，
  否则白噪声尾巴会把压下去的高频毛刺又补回来。
- 长音要加 3–6Hz/3–6% 的幅度微颤与 ±0.35% 音高 jitter：纯循环是"逐样本重复"，抖动为零，
  耳朵一听就是机器。
