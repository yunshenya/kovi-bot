...
#       recent_turns 带样本内匿名的 speaker 编号(a=当前发言者, b/c/… 给同群其他人)
...
### 群友编号（`context.recent_turns[].speaker`）

`role` 只有三档（`assistant` / `user` / `other_member`），于是一段上下文里的几个
其他群友全被标成"他人"——复核时看不出这是"一个人在连说三条"还是"三个人在互相
接话"，而这恰恰是判断 completion/response 的关键线索。所以采集器给 `recent_turns`
的每条再落一个 `speaker`：

- `a` 恒为**当前发言者**（`current_text` 与 `pending_user_fragments` 都属于他，
  所以这两处不需要各自带字段）；
- `b`/`c`/… 按出现顺序给同群的其他人，同一段上下文里同一个人拿同一个字母；
- 芸汐（`assistant`）不带 `speaker`——她不是"某个人"；
- 编号是**样本内匿名**的：只落序号，不落 QQ 号、昵称或任何跨样本稳定的标识，
  它只回答"这几条是不是同一个人"。脱敏承诺不变。

字段名是数据契约（叫 `speaker`），**界面上一律显示成"群友A/B/C…"**（每人一色）：
这些回合本来就是同一个群里的人。采于该字段之前的批次没有群友编号，网页端与
`review.py` 都退回按角色显示"同一人/他人"，**并且此时正文不再标"群友A"**——
上下文的名字编不出来，正文单独挂一个，同一个人就会同时叫"群友A"和"同一人"，
看着像两个人。网页端另有一行"群友未记录（这批采于编号之前）"说明原因。
注意"他人"只表示"不是当前发言者"：它是按 `sender` 判的，所以老批次里的每一条
"他人"确实都是别人，只是分不清几条是同一个。
